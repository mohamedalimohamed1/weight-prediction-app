...
# Klasörleri oluştur
os.makedirs("./visualization/decision_tree_graphs", exist_ok=True)
os.makedirs("./models/decision_tree_model", exist_ok=True)
...
# Pipeline: Ön işleme ve Decision Tree modelini birleştir
...
plt.plot(["Eğitim/Val", "Test"], [len(X_train), len(X_test)], marker='o', color="blue", linewidth=2)
plt.title("Veri Bölünmesi", fontsize=14)
plt.xlabel("Set Türü")
plt.ylabel("Örnek Sayısı")
plt.grid(True)
plt.savefig("./visualization/decision_tree_graphs/veri_bolunmesi.png")
plt.close()
...
model_path = "./models/decision_tree_model/decision_tree_model_pipeline.joblib"
...
for epoch in range(1, epochs + 1):
    ...
    confidence = test_r2 * 100  # Güven skoru yüzde olarak
...
plt.legend()
plt.grid(True)
plt.savefig("./visualization/decision_tree_graphs/egitim_sonuclari_grafik.png")
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig("./visualization/decision_tree_graphs/test_sonuclari_grafik.png")
plt.close()
print("Test sonuçları grafiği kaydedildi.")

# Modeli geri yükleme ve test etme
print("Kaydedilen model yükleniyor ve test ediliyor...")
loaded_model = joblib.load(model_path)
sample_prediction = loaded_model.predict(X_test[:5])
print(f"Örnek Tahminler: {sample_prediction}")

print("Model başarıyla eğitildi, kaydedildi ve test edildi.")
